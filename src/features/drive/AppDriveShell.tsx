  ...
  return (
    <div className="sticky top-0 z-[100] border-b border-gray-900/10 bg-white/80 backdrop-blur-sm dark:border-gray-500/30 dark:bg-gray-900">
      <div className="mx-auto flex w-full items-center justify-between px-4 py-2">
        ...
        <Link
          className="flex items-center space-x-2 rounded-sm bg-gray-100 px-3 py-1.5 text-sm text-gray-700 dark:bg-gray-800 dark:text-white"
...
function FileView({ file, path }: { file: OdFileObject; path: string }) {
  ...
  const rawUrl = `/api/raw/?path=${path}${hashedToken ? `&odpt=${hashedToken}` : ''}`
...
function AppDriveBrowser({ layout, path }: { layout: LayoutName; path: string }) {
  ...
  const responses = data.flat() as OdAPIResponse[]
  ...
  if ('folder' in firstResponse && firstResponse.folder) {
    const folderChildren = responses.flatMap(response => response.folder?.value ?? [])
    const isLoadingMore = size > 0 && data && typeof data[size - 1] === 'undefined'
    const lastPage = data[data.length - 1] as OdAPIResponse | undefined
    const isReachingEnd = !lastPage?.next
      ...
      <>
        ...
        {!onlyOnePage && (
          <div className="rounded-b bg-white dark:bg-gray-900 dark:text-gray-100">
            ...
            >
              {isLoadingMore ? 'Loading ...' : isReachingEnd ? 'No more files' : 'Load more'}
              {!isLoadingMore && !isReachingEnd && <FontAwesomeIcon icon={faChevronCircleDown} />}
            </button>
          </div>
        )}
      </>
  ...
  if ('file' in firstResponse && firstResponse.file) {
...
export default function AppDriveShell({ pathSegments }: { pathSegments: string[] }) {
    ...
    <div className="flex min-h-screen flex-col items-center justify-center bg-white dark:bg-gray-900">
      <main className="flex w-full flex-1 flex-col bg-gray-50 dark:bg-gray-800">
        ...
        <div className="mx-auto w-full max-w-5xl py-4 sm:p-4">
          ...
          <AppDriveBrowser layout={layout} path={path} />
        </div>
      </main>
      <Footer />
    </div>
